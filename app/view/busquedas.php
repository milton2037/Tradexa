    ...
    <section class="py-9 d-flex flex-column">
      ...
      <div class="row justify-content-center mb-5">
        <div class="col-lg-6">
          <form id="searchForm" class="input-group input-group-outline" method="POST" action="../../controller/buscar.php"> <!-- Enviar a buscar.php -->
            ...
            <input type="text" name="query" class="form-control" id="searchInput"> <!-- Añadido id para el input -->
      ...
      <div class="row flex-grow-1 w-100"> <!-- Añadido w-100 para ocupar todo el ancho disponible -->
          ...
          <div class="h-100 p-3" style="border-right: 1px solid rgba(0,0,0,0.1);">
            ...
            <div class="temas-scroll" style="height: 600px; overflow-y: scroll; scrollbar-color: rgba(0,0,0,0.1) transparent; scrollbar-width: thin;">
              <ul id="lista-temas">
                ...
                $query = "SELECT id, titulo FROM temas";
        ...
        <script>
          function obtenerContenidoPorId(idTema) {
            $.ajax({
              ...
              success: function(response) {
                $('#resultadoContenido').html(response);
              },
          ...
          function obtenerInformacionTema(idTema) {
            $.ajax({
              type: 'GET',
              url: '/app/controller/obtenerContenido.php', // Cambiar a ruta absoluta
              data: {
                id: idTema
              },
              success: function(response) {
                $('#resultadoBusqueda').html(response); // Mostrar la información en el contenedor de resultados
              },
              error: function(xhr, status, error) {
                console.error("Error en la solicitud: " + error);
              }
          ...
          function searchByTitle(title) {
            // Mostrar el tema seleccionado en la sección de búsqueda
            $.ajax({
              type: 'POST',
              url: '../controller/buscar.php', // Asegúrate de que la ruta sea correcta
              data: {
                query: title
              }, // Enviar el título del tema
              success: function(response) {
            ...
            <ul id="lista-temas">
              ...
              if ($resultTemas->num_rows > 0) {
                while ($tema = $resultTemas->fetch_assoc()) {
                  echo "<li><h2>" . $tema['titulo'] . "</h2> " . nl2br($tema['descripcion']) . "</li>"; // Añadido nl2br para saltos de línea
                  ...
                  if ($resultSubtemas->num_rows > 0) {
                    ...
                    while ($subtema = $resultSubtemas->fetch_assoc()) {
                      echo "<li><h3>" . $subtema['subtitulo'] . "</h3>: " . nl2br($subtema['descripcion']) . "</li>";
        ...
        <div class="col-md-2" style="height: 600px;"> <!-- Cambiado el tamaño del contenedor de subtemas -->
          <div class="h-100 p-2" style="border-left: 1px solid rgba(0,0,0,0.1); overflow-y: scroll; height: 1200px; scrollbar-color: rgba(0,0,0,0.1) transparent; scrollbar-width: thin;"> <!-- Aumentado height y modificado scrollbar -->
            <h3>Subtemas</h3>
            <ul id="lista-subtemas" style="text-align: left;"> <!-- Añadido estilo para justificar a la izquierda -->
              <?php
              ...
              if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                  ...
                  echo "<li style='text-align: left;'><button class='btn btn-link btn-sm' style='text-decoration: none; color: inherit; text-align: left' onclick=\"searchByTitle('" . $row['subtitulo'] . "')\">" . $row['subtitulo'] . "</button></li>";
                }
              } else {
